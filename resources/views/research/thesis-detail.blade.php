                ...
                <div class="lg:col-span-2 space-y-8">
                        ...
                        <div class="flex flex-wrap items-center gap-6">
                            <div class="flex items-center">
                                ...
                                <div>
                                    ...
                                    <p class="font-semibold text-[#26225C]">{{ $thesis->author }}</p>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center">
                                ...
                                <div>
                                    ...
                                    <p class="font-semibold text-[#26225C]">{{ $thesis->approved_at ? $thesis->approved_at->format('F j, Y') : 'N/A' }}</p>
                                </div>
                            </div>
                            @endif
                            ...
                            <div class="flex items-center">
                                ...
                                <div>
                                    ...
                                    <p class="font-semibold text-[#26225C]">{{ $thesis->year_completed }}</p>
                                </div>
                            </div>
                        </div>
                    </section>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                                <div class="p-6 border-l-4 border-[#FFC72C] bg-gray-50 rounded-lg">
                                    ...
                                    <p class="text-gray-700 font-medium">{{ $thesis->department }}</p>
                                </div>
                                <div class="p-6 border-l-4 border-[#FFC72C] bg-gray-50 rounded-lg">
                                    <div class="flex items-center mb-3">
                                        <div class="w-10 h-10 bg-[#26225C] bg-opacity-10 rounded-lg flex items-center justify-center mr-3">
                                    ...
                                    <p class="text-gray-700 font-medium">Master's Degree</p>
                                </div>
                            </div>
                    ...
                    <section class="mb-8">
                        ...
                        <div class="bg-gray-50 rounded-lg border-l-4 border-[#FFC72C] overflow-hidden">
                            ...
                            <div class="p-6">
                                @php
                                    $year = $thesis->approved_at ? $thesis->approved_at->format('Y') : $thesis->year_completed;
                                    $date = $thesis->approved_at ? $thesis->approved_at->format('d M. Y') : date('d M. Y', strtotime($thesis->year_completed . '-01-01'));
                                @endphp

                                <!-- APA Format -->
                                <div id="citation-content-apa" class="citation-content">
                            <p class="text-sm text-gray-700 font-mono mb-4 leading-relaxed">
                                        {{ $thesis->author }} ({{ $year }}). <em>{{ $thesis->title }}</em> (Master's thesis). 
                                ...
                                <div id="citation-content-mla" class="citation-content hidden">
                                    <p class="text-sm text-gray-700 font-mono mb-4 leading-relaxed">
                                        {{ $thesis->author }}. "{{ $thesis->title }}." Master's thesis, University of Southern Philippines Foundation, 
                                        {{ $thesis->department }}, {{ $date }}, 
                                        {{ url()->current() }}.
                                    </p>
                                </div>
                                ...
                                <div id="citation-content-chicago" class="citation-content hidden">
                                    <p class="text-sm text-gray-700 font-mono mb-4 leading-relaxed">
                                        {{ $thesis->author }}. "{{ $thesis->title }}." Master's thesis, {{ $thesis->department }}, 
                                ...
                                <div id="citation-content-harvard" class="citation-content hidden">
                                    <p class="text-sm text-gray-700 font-mono mb-4 leading-relaxed">
                                        {{ $thesis->author }} {{ $year }}, <em>{{ $thesis->title }}</em>, Master's thesis, University of Southern Philippines Foundation, 
                                        {{ $thesis->department }}, viewed {{ date('d M Y') }}, &lt;{{ url()->current() }}&gt;.
                                    </p>
                                </div>
                    ...
                    <section>
                        ...
                        <div class="space-y-4">
                            ...
                            @if($thesis->abstract_file)
                                ...
                                @if(auth()->check())
                                <a href="{{ route('thesis.download-abstract.file', $thesis->id) }}" 
                            ...
                            @if($thesis->document_file && $isAdmin)
                                ...
                                <a href="{{ route('thesis.download.file', $thesis->id) }}" 
                                   class="flex items-center justify-center w-full px-6 py-3 bg-[#26225C] hover:bg-[#3a3770] text-white font-medium rounded-xl transition-colors">
                                    <svg class="h-5 w-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                                    </svg>
                    ...
                    <section>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="flex justify-between items-center py-3">
                                ...
                                <span class="font-semibold text-[#26225C] text-xl">{{ $thesis->approved_at->diffForHumans() }}</span>
                            </div>
                            @endif
                        </div>
                    </section>
                        ...
                        <div class="flex items-center">
                            <div class="h-16 w-16 bg-[#26225C] bg-opacity-10 rounded-xl flex items-center justify-center">
                                <span class="text-[#26225C] font-semibold text-xl">
                                    {{ substr($thesis->user->name ?? 'NA', 0, 2) }}
                                </span>
                            </div>
                            <div class="ml-4">
                                <p class="font-semibold text-[#26225C] text-lg">{{ $thesis->user->name ?? 'N/A' }}</p>
                                <p class="text-sm text-gray-600">{{ $thesis->user->email ?? 'N/A' }}</p>
    ...
    <script>
        ...
        function shareResearch() {
            if (navigator.share) {
                navigator.share({
                    title: '{{ $thesis->title }}',
                    text: 'Check out this research: {{ $thesis->title }}',
        ...
        function loadCitations(type) {
            const url = type === 'cited-by' 
                ? '/my-citations?filter=citing_research_title:{{ urlencode($thesis->title) }}' 
                : '/research-citations/thesis/{{ $thesis->id }}';
