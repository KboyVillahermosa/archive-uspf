<x-app-layout>
            ...
            <div class="flex flex-col lg:flex-row gap-6">
                ...
                <div class="flex-1 space-y-4">
                    ...
                    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
                        ...
                        <div class="h-max bg-[#26225C] relative">
                             @if($thesis->banner_image)
                                <img src="{{ asset('storage/' . $thesis->banner_image) }}" alt="Research Banner" class="w-full h-48 object-cover opacity-80">
                            @else
                        ...
                        <div class="px-8 pb-8 pt-6 relative">
                            <div class="flex items-center gap-2 mb-4">
                                <span class="px-2.5 py-0.5 bg-orange-50 text-orange-600 border border-orange-100 rounded text-[10px] font-black uppercase tracking-wider">
                                    Master's Thesis
                                </span>
                                <span class="px-2.5 py-0.5 bg-gray-50 text-gray-400 border border-gray-100 rounded text-[10px] font-bold uppercase">
                                    Archive #T-{{ str_pad($thesis->id, 5, '0', STR_PAD_LEFT) }}
                                </span>
                            </div>
                            ...
                            <div class="flex items-center gap-4 mt-6">
                                <div class="w-12 h-12 bg-gray-100 rounded font-black text-[#26225C] flex items-center justify-center border border-gray-100 text-lg">
                                    {{ substr($thesis->author ?? 'T', 0, 1) }}
                                </div>
                                <div class="min-w-0">
                                    <h4 class="text-base font-black text-gray-900 truncate">{{ $thesis->author ?? 'Researcher' }}</h4>
                                    <p class="text-[11px] font-bold text-gray-500 uppercase tracking-widest mt-0.5">
                                        {{ $thesis->department }} • Graduate Student
                                    </p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="bg-gray-50/50 rounded-lg border border-gray-100 overflow-hidden">
                                ...
                                <div class="p-6">
                                    <div id="citation-container" class="text-[12px] font-mono text-gray-600 leading-relaxed mb-4">
                                        @include('research.partials.citations', ['research' => $thesis, 'type' => 'thesis'])
                                    </div>
                ...
                <div class="lg:w-80 space-y-4">
                    <div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm space-y-3">
                        ...
                        @if($thesis->thesis_file && (auth()->check() || $isAdmin))
                            <a href="{{ route('thesis.view.pdf', $thesis->id) }}" target="_blank"
                    ...
                    <div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm space-y-4">
                        ...
                        <div class="space-y-3">
                            <div class="flex justify-between items-center py-2 border-b border-gray-50">
                                ...
                                <span class="text-[11px] font-black text-[#26225C] uppercase tracking-tighter">{{ $thesis->department }}</span>
                            </div>
                            <div class="flex justify-between items-center py-2 border-b border-gray-50">
                                <span class="text-[10px] font-bold text-gray-400 uppercase tracking-tight">Level</span>
                                <span class="text-[11px] font-black text-[#26225C] uppercase tracking-tighter">Master's</span>
                            </div>
                            <div class="flex justify-between items-center py-2 border-b border-gray-50">
                                <span class="text-[10px] font-bold text-gray-400 uppercase tracking-tight">Published</span>
                                <span class="text-[11px] font-black text-gray-600 uppercase">{{ $thesis->approved_at ? $thesis->approved_at->format('M d, Y') : 'N/A' }}</span>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        function shareResearch() {
            if (navigator.share) {
                navigator.share({ title: '{{ $thesis->title }}', url: window.location.href });
            } else {
        ...
        function loadCitations(type) {
            const url = type === 'cited-by' 
                ? '/references-cited/thesis/{{ $thesis->id }}' 
