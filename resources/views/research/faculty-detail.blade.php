<x-app-layout>
            ...
            <div class="flex flex-col lg:flex-row gap-6">
                ...
                <div class="flex-1 space-y-4">
                    ...
                    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
                        ...
                        <div class="px-8 pb-8 pt-6 relative">
                            <div class="flex items-center gap-2 mb-4">
                                <span class="px-2.5 py-0.5 bg-purple-50 text-purple-600 border border-purple-100 rounded text-[10px] font-black uppercase tracking-wider">
                                    Faculty Research
                                </span>
                                <span class="px-2.5 py-0.5 bg-gray-50 text-gray-400 border border-gray-100 rounded text-[10px] font-bold uppercase">
                                    Archive #F-{{ str_pad($research->id, 5, '0', STR_PAD_LEFT) }}
                                </span>
                            </div>
                            ...
                            <div class="flex items-center gap-4 mt-6">
                                <div class="w-12 h-12 bg-gray-100 rounded font-black text-[#26225C] flex items-center justify-center border border-gray-100 text-lg">
                                    {{ substr($research->user->name ?? 'F', 0, 1) }}
                                </div>
                                <div class="min-w-0">
                                    <h4 class="text-base font-black text-gray-900 truncate">{{ $research->user->name ?? 'Faculty Member' }}</h4>
                                    <p class="text-[11px] font-bold text-gray-500 uppercase tracking-widest mt-0.5">
                                        {{ $research->department }} • Lead Researcher
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="bg-white rounded-xl border border-gray-200 p-8 shadow-sm space-y-10">
                        ...
                        <section>
                            <h3 class="text-[11px] font-black text-[#26225C] uppercase tracking-[0.2em] mb-4 border-b border-gray-100 pb-2">Collaborators</h3>
                            <p class="text-[13px] text-gray-600 font-medium">
                                {{ $research->co_researchers }}
                            </p>
                        </section>
                        @endif
                        ...
                        <section>
                            ...
                            <div class="flex flex-wrap gap-2">
                                @foreach(explode(',', $research->keywords) as $tag)
                            ...
                            <div class="bg-gray-50/50 rounded-lg border border-gray-100 overflow-hidden">
                                ...
                                <div class="p-6">
                                    <div id="citation-container" class="text-[12px] font-mono text-gray-600 leading-relaxed mb-4">
                                        @include('research.partials.citations', ['research' => $research, 'type' => 'faculty'])
                                    </div>
                ...
                <div class="lg:w-80 space-y-4">
                    <div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm space-y-3">
                        ...
                        @if($research->abstract_file)
                            <a href="{{ route('faculty.view-abstract.pdf', $research->id) }}" 
                        ...
                        @if($research->document_file && (auth()->check() || $isAdmin))
                            <a href="{{ route('faculty.view.pdf', $research->id) }}" target="_blank"
    ...
    <script>
        ...
        function shareResearch() {
            if (navigator.share) {
                navigator.share({ title: '{{ $research->title }}', url: window.location.href });
            } else {
        ...
        function loadCitations(type) {
            const url = type === 'cited-by' 
                ? '/references-cited/faculty/{{ $research->id }}' 
