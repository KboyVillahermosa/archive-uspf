    ...
    <aside :class="expanded ? 'w-64' : 'w-20'" 
        ...
        <nav class="flex-1 py-6 space-y-2 overflow-y-auto" :class="expanded ? 'px-4' : 'px-2'">
            ...
            @if($isFaculty)
                ...
                <a href="{{ route('admin.adviser-approvals') }}" 
                   class="flex items-center text-sm font-medium rounded-lg transition-all duration-200 group {{ request()->routeIs('admin.adviser-approvals') ? 'bg-[#FFC72C] text-[#26225C] shadow-md' : 'text-white hover:bg-[#1a1840] hover:text-[#FFC72C]' }}"
                   :class="expanded ? 'px-4 py-3 justify-start' : 'px-2 py-3 justify-center'">
                    <svg class="w-5 h-5 flex-shrink-0" :class="expanded ? 'mr-3' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <span x-show="expanded" 
                          ...
                          x-transition:leave-start="opacity-100" 
                          x-transition:leave-end="opacity-0"
                          class="whitespace-nowrap">Adviser Approvals</span>
                </a>
            @endif
        </nav>
    ...
    <aside x-show="mobileOpen" 
        ...
        <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
            ...
            @if($isFaculty)
                ...
                <a href="{{ route('admin.adviser-approvals') }}" 
                   @click="mobileOpen = false"
                   class="flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-all duration-200 {{ request()->routeIs('admin.adviser-approvals') ? 'bg-[#FFC72C] text-[#26225C] shadow-md' : 'text-white hover:bg-[#1a1840] hover:text-yellow-300' }}">
                    <svg class="w-5 h-5 mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                    </svg>
                    <span>Adviser Approvals</span>
                </a>
            @endif
        </nav>
