<x-app-layout>
    <div class="min-h-screen bg-gray-50">
        <div class="w-full max-w-full mx-auto px-2 sm:px-4 lg:px-6 py-8">
            ...
            @if(isset($isAdmin) && $isAdmin && isset($adviserPendingResearch) && $adviserPendingResearch['total_count'] > 0)
            <!-- Adviser Approval Reminders -->
            <div class="mt-8">
                <div class="mb-6 pb-4 border-b-2 border-[#FFC72C]">
                    <h2 class="text-3xl font-bold text-[#26225C] mb-1">Research Waiting for Adviser Approval</h2>
                    <p class="text-sm text-gray-600">These research items have assigned advisers but are waiting for adviser approval. You can still approve them independently.</p>
                </div>
                ...
                <div class="bg-yellow-50 border-l-4 border-yellow-400 rounded-lg p-6 mb-6">
                    <div class="flex items-start">
                        ...
                        <div>
                            <h3 class="text-lg font-semibold text-yellow-800 mb-1">Reminder</h3>
                            <p class="text-sm text-yellow-700">This is a reminder only. You can still approve these research items even if the adviser hasn't approved yet.</p>
                        </div>
                    </div>
                </div>

                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    @if($adviserPendingResearch['student']->isNotEmpty())
                    <div class="bg-white rounded-xl shadow-lg border border-gray-200">
                        <div class="bg-gradient-to-r from-[#26225C] to-[#3a3770] text-white px-6 py-4 rounded-t-xl">
                            <h3 class="text-lg font-semibold">Student Research ({{ $adviserPendingResearch['student']->count() }})</h3>
                        </div>
                        <div class="p-6">
                            <div class="space-y-3 max-h-96 overflow-y-auto">
                                @foreach($adviserPendingResearch['student'] as $research)
                                <div class="border-l-4 border-[#26225C] pl-4 py-2 hover:bg-gray-50 transition-colors rounded-r">
                                    <div class="flex items-start justify-between">
                                        <div class="flex-1">
                                            ...
                                            <div class="text-xs text-gray-600">
                                                <span class="font-medium">Adviser:</span> {{ $research->adviser->name ?? $research->adviser->email }}
                                            </div>
                                            <div class="text-xs text-gray-500 mt-1">
                                                Waiting {{ $research->created_at->diffForHumans() }}
                                            </div>
                                        </div>
                                        <a href="{{ route('admin.approve.student.form', $research->id) }}" class="ml-4 text-[#26225C] hover:text-[#FFC72C] text-sm font-medium">
                                            Review →
                                        </a>
                                    </div>
                                </div>
                    ...
                    @if($adviserPendingResearch['faculty']->isNotEmpty())
                    <div class="bg-white rounded-xl shadow-lg border border-gray-200">
                        <div class="bg-gradient-to-r from-purple-600 to-purple-800 text-white px-6 py-4 rounded-t-xl">
                            <h3 class="text-lg font-semibold">Faculty Research ({{ $adviserPendingResearch['faculty']->count() }})</h3>
                        </div>
                        <div class="p-6">
                            <div class="space-y-3 max-h-96 overflow-y-auto">
                                @foreach($adviserPendingResearch['faculty'] as $research)
                                <div class="border-l-4 border-purple-600 pl-4 py-2 hover:bg-gray-50 transition-colors rounded-r">
                                    <div class="flex items-start justify-between">
                                        <div class="flex-1">
                                            <div class="text-sm font-medium text-gray-900 mb-1">
                                                {{ Str::limit($research->title, 50) }}
                                        ...
                                        <a href="{{ route('admin.approve.faculty.form', $research->id) }}" class="ml-4 text-purple-600 hover:text-purple-800 text-sm font-medium">
                                            Review →
                                        </a>
                                    </div>
                                </div>
                    ...
                    @if($adviserPendingResearch['thesis']->isNotEmpty())
                    <div class="bg-white rounded-xl shadow-lg border border-gray-200">
                        <div class="bg-gradient-to-r from-green-600 to-green-800 text-white px-6 py-4 rounded-t-xl">
                            <h3 class="text-lg font-semibold">Master's Thesis ({{ $adviserPendingResearch['thesis']->count() }})</h3>
                        </div>
                        <div class="p-6">
                            <div class="space-y-3 max-h-96 overflow-y-auto">
                                @foreach($adviserPendingResearch['thesis'] as $research)
                                <div class="border-l-4 border-green-600 pl-4 py-2 hover:bg-gray-50 transition-colors rounded-r">
                                    <div class="flex items-start justify-between">
                                        <div class="flex-1">
                                            <div class="text-sm font-medium text-gray-900 mb-1">
                                                {{ Str::limit($research->title, 50) }}
                                        ...
                                        <a href="{{ route('admin.approve.thesis.form', $research->id) }}" class="ml-4 text-green-600 hover:text-green-800 text-sm font-medium">
                                            Review →
                                        </a>
                                    </div>
                                </div>
                    ...
                    @if($adviserPendingResearch['dissertations']->isNotEmpty())
                    <div class="bg-white rounded-xl shadow-lg border border-gray-200">
                        <div class="bg-gradient-to-r from-red-600 to-red-800 text-white px-6 py-4 rounded-t-xl">
                            <h3 class="text-lg font-semibold">Dissertations ({{ $adviserPendingResearch['dissertations']->count() }})</h3>
                        </div>
                        <div class="p-6">
                            <div class="space-y-3 max-h-96 overflow-y-auto">
                                @foreach($adviserPendingResearch['dissertations'] as $research)
                                <div class="border-l-4 border-red-600 pl-4 py-2 hover:bg-gray-50 transition-colors rounded-r">
                                    <div class="flex items-start justify-between">
                                        <div class="flex-1">
                                            <div class="text-sm font-medium text-gray-900 mb-1">
                                                {{ Str::limit($research->title, 50) }}
                                        ...
                                        <a href="{{ route('admin.approve.dissertation.form', $research->id) }}" class="ml-4 text-red-600 hover:text-red-800 text-sm font-medium">
                                            Review →
                                        </a>
                                    </div>
                                </div>
